...
st.set_page_config(
    page_title="IkigAI V1.73 - Pure Executive Hub", 
    page_icon="🧬", 
    layout="wide",
    initial_sidebar_state="expanded"
)

# Estilo CSS V1.73: Zen Minimalista con Contraste Quirúrgico
st.markdown("""
    <style>
...
# --- 5. BARRA LATERAL (ZEN V1.59 + RESET ATÓMICO) ---
with st.sidebar:
...
for i, msg in enumerate(st.session_state.get("messages", [])):
    with st.chat_message(msg["role"]):
        ...
        if msg["role"] == "assistant":
            if st.button(f"📋 Copiar", key=f"cp_{i}"):
