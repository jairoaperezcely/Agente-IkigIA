...
st.set_page_config(page_title="IkigAI V2.0 - Executive Hub", page_icon="🧬", layout="wide")
...
# --- 3. MOTORES DE LECTURA MULTIFORMATO ---
...
# --- 5. BARRA LATERAL RECONFIGURADA ---
with st.sidebar:
    st.markdown("<h1 style='text-align: center; color: #00E6FF;'>🧬</h1>", unsafe_allow_html=True)
    rol_activo = st.radio("Rol Estratégico:", options=list(ROLES.keys()), label_visibility="collapsed")
    ...
    with t_doc:
        up = st.file_uploader("Sidebar (Análisis Volátil):", type=['pdf', 'docx', 'pptx'], accept_multiple_files=True)
        if st.button("🧠 Procesar Sidebar", use_container_width=True):
            ...
            st.session_state.biblioteca[rol_activo] = raw[:40000]; st.success("Cargado.")
    ...
    with t_url:
        ...
        if st.button("🌐 Scrapear", use_container_width=True):
            try:
                ...
                st.session_state.biblioteca[rol_activo] = BeautifulSoup(res.text, 'html.parser').get_text()[:30000]; st.success("Web lista.")
    ...
    with t_img:
        ...
        if up_i and st.button("👁️ Visión AI", use_container_width=True):
            img = Image.open(up_i)
            res_v = genai.GenerativeModel('gemini-1.5-flash-latest').generate_content(["Análisis estratégico:", img])
            st.session_state.biblioteca[rol_activo] = res_v.text; st.success("Imagen procesada.")

    # Curaduría: Consagración
    if st.session_state.biblioteca[rol_activo]:
        st.markdown("<div class='section-tag'>Curaduría</div>", unsafe_allow_html=True)
        nombre_c = st.text_input("Guardar como:", value="Documento_Curado.txt")
        if st.button("📌 Consagrar a Máster"):
            ...
            actualizar_memoria_persistente(); st.success("Blindado en Máster.")
    ...
    with st.chat_message("assistant"):
        try:
            ...
            if os.path.exists(DB_NATIVO):
                with open(DB_NATIVO, "r", encoding="utf-8") as f: master = json.load(f)
                keys = pr.lower().split()
                m = []
                for it in master:
            ...
            sys = f"Actúa como {rol_activo}. ROI Cognitivo. MÁSTER: {c_rag[:15000]}. SIDEBAR: {c_sid[:15000]}. Si hay conflicto entre fuentes, señala '⚠️ ALERTA DE CHOQUE'."
